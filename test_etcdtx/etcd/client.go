...
import (
	...
	"github.com/eechains/scaring/pkg/core/logger"
...
// TODO @江金 在get 和 set 方法上 添加重试获取失败后,主动尝试重新获取
...
func (e *Client) getToMap(context context.Context, key string, opts ...clientv3.OpOption) (values map[string]string, err error) {
	resp, err := e.Client.Get(context, key, opts...)
	if err != nil {
		return values, err
	}
	...
	for i := 0; i < length; i++ {
		values[string(s[i].Key)] = string(s[i].Value)
	}
...
func (e *Client) delete(context context.Context, key string, opts ...clientv3.OpOption) error {
	if key == "" {
		return argError
	}
	_, err := e.Client.Delete(context, key, opts...)
	return err
}
...
func (e *Client) Delete(key string, opts ...clientv3.OpOption) error {
	return e.delete(context.TODO(), key, opts...)
}

func (e *Client) DeleteWithTimeOut(key string, opts ...clientv3.OpOption) error {
	ctx, cancel := context.WithTimeout(context.Background(), e.Config.RequestTimeOut)
	defer cancel()
	err := e.delete(ctx, key, opts...)
	return err
}

func (e *Client) GetToMap(key string, opts ...clientv3.OpOption) (map[string]string, error) {
	return e.getToMap(context.TODO(), key, opts...)
}
...
func (e *Client) GetToMapWithTimeOut(key string, opts ...clientv3.OpOption) (map[string]string, error) {
	ctx, cancel := context.WithTimeout(context.Background(), e.Config.RequestTimeOut)
	defer cancel()
	values, err := e.getToMap(ctx, key, opts...)
	return values, err
}
...
// 分布式锁
func (e *Client) NewMutex(prefixKey string) (*concurrency.Mutex, error) {
	// 默认
	session, err := concurrency.NewSession(e.Client)
	if err != nil {
		return nil, err
	}
	mux := concurrency.NewMutex(session, prefixKey)
	return mux, nil
}
